...
const DetailLogsPage = ({ params }: { params: { clientId: string } }) => {
    ...
    const data = useSelector(
        (state: RootState) => state.testBlasting.detailLogs
    );
    const status = useSelector(
        (state: RootState) => state.testBlasting.statusDetailLogs
    );
        ...
        <>
            <div className="flex items-center justify-between mb-3">
                <h1 className="font-semibold text-2xl">
                    Detail Log Test Blasting
                </h1>
                <BreadcrumbsComponent data={BreadCrumbsData} />
            </div>
            <div
                className="tooltip tooltip-bottom mb-5"
                data-tip="Back to testing"
            >
                <Link
                    href="/dashboard/test-blasting"
                    className="btn bg-slate-900 font-semibold text-white"
                >
                    <ArrowUturnLeftIcon />
                </Link>
            </div>
