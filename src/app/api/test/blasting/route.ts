...
import { createLogs, createQuestion, getLogs, sendMessage } from '@/services/testBlastingService';
import { faker } from '@faker-js/faker';
import { NextRequest, NextResponse } from 'next/server';

export async function POST(req: NextRequest) {
   const { access_token, target_phone_number, event_name, sender, guest_name, invitation_link } = await req.json();
   ...
   try {
      ...
      const questionData = [
         ...
         {
            question: '<p>This invitation is valid for <strong>2 Guest(s)</strong>, how many guest will attend?</p>',
            type: 'number',
            flag: 'normal_question',
            position: 2,
         },
         {
            question: '<p>Are any guest vegetarian? (Optional) Example: Bambang - Vegetarian / Adeline Vegetarian</p>',
            type: 'text',
            flag: 'normal_question',
            position: 3,
         },
