...
export async function POST(req: NextRequest) {
   ...
   const response = await createUser(jwtToken as string, username, email, password, createdBy, updatedBy);
   return NextResponse.json(response, {
      status: response.status,
   });
}
...
export async function PUT(req: NextRequest) {
   const { id, username, email, password, createdBy, updatedBy } = await req.json();
   const token = req.headers.get('authorization');
   const jwtToken = token?.split(' ')[1];
   ...
   const response = await updateUser(jwtToken as string, id, username, email, password, createdBy, updatedBy);
