...
export const InvitationHome = ({ invitations }: InvitationHomeProps) => {
  ...
  const [q1Answer, setQ1Answer] = useState<string>(''); // Jawaban main question
  ...
  // ============================================================
  // IDENTIFIKASI MAIN QUESTION
  // ============================================================
  const mainQuestion = invitations?.data?.questions?.find((q: any) => q.position === 1);
  const mainQuestionId = mainQuestion?.id?.toString();
  ...
  const submitAnswer = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      for (const data of newData) {
        await dispatch(putAnswer(data as any)).unwrap();
      }
      Swal.fire({
        icon: 'success',
        title: 'Success',
  ...
  // ============================================================
  // HANDLE RADIO BUTTON (DENGAN KONTROL Q1)
  // ============================================================
  const handleChangeRadio = (e: React.ChangeEvent<HTMLInputElement>) => {
    const questionId = e.target.name.split('_')[1];
    const selectedValue = e.target.value;
    const isMainQuestionClick = questionId === mainQuestionId;

    // Kalau bukan main question
    if (!isMainQuestionClick) {
      // Jika Q1 belum dijawab atau Q1 = "no" → blok interaksi
      if (q1Answer === '' || q1Answer === 'no') {
        e.preventDefault();
        return;
      }

      // Simpan nilai lokal untuk radio ini
      setFormValues((prev: any) => ({
        ...prev,
        [`question_${questionId}`]: selectedValue,
      }));
      return;
    }

    // === Kalau ini MAIN QUESTION ===
    ...
    // Jika Q1 = NO → reset semua kecuali Q1
    if (selectedValue === 'no') {
      const resetFormValues: any = {};
      resetFormValues[`question_${mainQuestionId}`] = 'no';
  ...
  const rawHTML = (
    <p>
      Please confirm your attendance, <strong>YES</strong> (joyfully accept),
              ...
              {invitations?.data?.questions?.map(
                (question: any, index: number) => {
                  const isMainQuestion =
                    question.id.toString() === mainQuestionId;

                  // LOGIKA ENABLE/DISABLE
                  let shouldDisable = false;
                  if (!isMainQuestion) {
                    if (q1Answer !== 'yes') shouldDisable = true;
                  }
                      ...
                      {question.type === 'radio' ? (
                        <div className="flex items-center gap-4">
                          ...
                          <div className="flex items-center gap-2">
                            <input
                              className="radio radio-sm checked:bg-[#1C1C1C] border-[#1C1C1C] disabled:opacity-50 disabled:cursor-not-allowed"
                              type="radio"
                              name={`question_${question.id}`}
                              id={`question_yes_${question.id}`}
                              value="yes"
                              onChange={
                                shouldDisable ? undefined : handleChangeRadio
                              }
                              onClick={
                                shouldDisable
                                  ? (e) => e.preventDefault()
                              ...
                              checked={
                                formValues[`question_${question.id}`] === 'yes'
                              }
                            />
                            <label
                              htmlFor={`question_yes_${question.id}`}
                              className={
                                shouldDisable
                          ...
                          <div className="flex items-center gap-2">
                            <input
                              ...
                              disabled={shouldDisable}
                              checked={
                                formValues[`question_${question.id}`] === 'no'
                              }
                            />
                            <label
                              htmlFor={`question_no_${question.id}`}
                              className={
                                shouldDisable
                                  ? 'opacity-50 cursor-not-allowed'
                                  : 'cursor-pointer'
                        ...
                        <input
                          type="text"
                          className={`rounded-md px-3 py-2 disabled:opacity-50 disabled:cursor-not-allowed disabled:bg-gray-200 ${
