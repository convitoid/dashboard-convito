...
import { createUser, fetchUsers } from '@/app/GlobalRedux/Features/user/userSlicer';
         ...
         >
            <form onSubmit={(e) => submitHandler(e)}>
               ...
               <div className="flex items-center justify-end gap-1 mt-4">
                  <button className="btn bg-slate-300" type="button" onClick={() => closeModalEdit()}>
                     Cancel
                  </button>
                  <button className={`btn bg-blue-700 text-white hover:bg-blue-800 `} disabled={status === 'loading'}>
                     {status === 'loading' ? <span className="loading loading-spinner loading-sm"></span> : 'Save'}
                  </button>
               </div>
            </form>
         </ModalComponent>
